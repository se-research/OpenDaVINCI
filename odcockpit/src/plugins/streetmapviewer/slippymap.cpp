...
SlippyMap::SlippyMap(QObject *prnt)
    : QObject(prnt), width(400), height(300), zoom(16),
      latitude(59.9138204), longitude(10.7387413),
      m_offset(), m_tilesRect(), m_emptyTile(), m_tilePixmaps(),
      m_manager(), m_url()
{
...
void SlippyMap::handleNetworkData(QNetworkReply *reply)
{
    ...
    foreach(QPoint tp2, m_tilePixmaps.keys())
    if (!bound.contains(tp2))
        m_tilePixmaps.remove(tp2);
