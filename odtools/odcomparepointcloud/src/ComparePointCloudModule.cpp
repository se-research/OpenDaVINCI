...
namespace odcomparepointcloud {
    ...
    odcore::data::dmcp::ModuleExitCodeMessage::ModuleExitCode ComparePointCloudModule::body() {
        ...
        unique_ptr< NonReorderingPlayer > player;
        //player = unique_ptr< NonReorderingPlayer >(new Player(url, AUTO_REWIND, MEMORY_SEGMENT_SIZE, NUMBER_OF_SEGMENTS, THREADING));
        player = unique_ptr< NonReorderingPlayer >(new Player(url, 0, 2800000, 20, false));
        Container c;
        Container cpcFrame;
        Container spcFrame;
