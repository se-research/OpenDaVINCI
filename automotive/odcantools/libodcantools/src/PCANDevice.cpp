...
namespace automotive {
    namespace odcantools {
        ...
        PCANDevice::PCANDevice(const string &deviceNode, GenericCANMessageListener &listener) :
            CANDevice(),
            ...
            CLOG << "[PCANDevice] Opening " << m_deviceNode << "... ";
...

                {
                    ...
                    if (pcanStartTimeProcFile.good()) {
                        ...
                        if (tokens.size() == 2) {
                            ...
                            CLOG << "[PCANDevice] Kernel module start time: " << m_deviceDriverStartTime.getYYYYMMDD_HHMMSSms() << endl;
                        }
                    }
                }
            }
        ...
        PCANDevice::~PCANDevice() {
            CLOG << "[PCANDevice] Closing " << m_deviceNode << "... ";
            if (m_handle != NULL) {
                CAN_Close(m_handle);
            }
            CLOG << "done." << endl;
        ...
        int PCANDevice::write(const GenericCANMessage &gcm) {
            int errorCode = 0;
            ...
            if (m_handle != NULL) {
                ...
                if (0 == errorCode) {
                    CLOG1 << "[PCANDevice] Writing ID = " << msg.ID << ", LEN = " << +msg.LEN << ", errorCode = " << errorCode << endl;
                }
                else {
                    CLOG1 << "[PCANDevice] Writing ID = " << msg.ID << ", LEN = " << +msg.LEN << ", errorCode = " << errorCode << ", strerror(" << errno << "): '" << strerror(errno) << "'" << endl;
                }
            }
            return errorCode;
        }

        void PCANDevice::beforeStop() {}
        ...
        void PCANDevice::run() {
            ...
            while ( (m_handle != NULL) && 
                ...
                if ( !(errorCode < 0) && (errorCode != CAN_ERR_QRCVEMPTY) ) {
                    CLOG1 << "[PCANDevice] ID = " << message.Msg.ID << ", LEN = " << message.Msg.LEN << ", DATA = ";
